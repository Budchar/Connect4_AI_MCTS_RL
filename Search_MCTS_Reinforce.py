...
class Agamotto(game):
    ...
    def VictoryNumber(self):
        ...
        for battle_field in battle_list:
            ...
            self.FutureSight(battle_field)
            while sum(self.EOA[next_battle]) < 30:
                self.FutureSight(battle_field)
            if self.marker == 'X':
                win = self.EOA[next_battle][0]
                lose = self.EOA[next_battle][2]
                draw = self.EOA[next_battle][1]
                winrate =  win / (win+lose+draw)
            elif self.marker == "O":
                win = self.EOA[next_battle][2]
                lose = self.EOA[next_battle][0]
                draw = self.EOA[next_battle][1]
                winrate = win / (win + lose + draw)
            battle_winrate_list.append((battle_field, next_battle, winrate))
            print(f'Dr.Weird: {battle_field}를 선택한 {next_battle}전장의 전투는 {win+lose+draw}번, 승리 {win}, 패배 {lose}, 무승부 {draw}, 승률은 {winrate*100:0.2f}%네')
        vic_num = str(max(battle_winrate_list, key=lambda x: x[2])[0])[0]
        print(f"Dr.Weird: {self.BC}개의 전투에서 승리할 수 있는 수는 {vic_num}뿐이었네")
        return vic_num

    def FutureSight(self, code):
        orb = deepcopy(self)
        str_len = len(orb.battle)
        orb.input(orb.validate_input_col(str(code)))
        ...
        if orb.winner == 'X':
            ...
            for i in range(str_len, len(orb.battle)):
